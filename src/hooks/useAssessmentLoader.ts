      ...
      try {
        ...
        const fetchWithTimeout = async () => {
          ...
          const { data: assessmentData, error: assessmentError } =
            (await Promise.race([assessmentPromise, timeoutPromise])) as any;

          if (assessmentError || !assessmentData) {
            throw new Error(
              assessmentError?.message || "Avaliação não encontrada"
            );
          }
        ...
        const fetchQuestions = async (retryCount = 0) => {
          try {
            ...
            if (questionsError) {
              throw new Error(
                questionsError.message || "Erro ao carregar questões"
              );
            }
        ...
        const formattedQuestions: AssessmentQuestion[] =
          questionsData.map(mapQuestionType);
...
const mapQuestionType = (questao: any): AssessmentQuestion => {
  ...
  switch (questao.question_type) {
    ...
    case "matching":
      const correspondencias = getJsonProperty<
        Array<{ left: string; right: string }>
      >(questao.options, "matches");
      return correspondencias
        ? { ...tipoBase, matches: correspondencias }
        : tipoBase;
